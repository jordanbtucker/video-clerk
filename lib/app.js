...
module.exports = class App {
  ...
  async run() {
      ...
      },
      {
        ...
        choices: async () => {
          ...
          return names
            .filter(n =>
              n.match(
                /\.(avi|flv|m4[pv]|mp([24egv]|eg)|mkv|ogg|swf|webm|wmv)$/i,
              ),
            )
            .sort()
        },
    ...
    // Movie filenames are matched by a title followed by a year and separated
    // by dots. For example, The.Dark.Knight.2008.1080p.mkv
    const movieTitleRegExp = /^(.+)\.(\d{4})\./

    // TV show filenames are matched by a title followed by a season and episode
    // number separated by dots. For example, Harley.Quinn.S02E04.720p.mkv
    const showTitleRegExp = /^(.+)\.S(\d\d)E(\d\d)\./
      ...
      if (searchData.results && searchData.results.length) {
        ...
        if (searchData.results.length === 1) {
          ...
          const resultID = await promptList(
            `Multiple results found for ${title}. Please select one.`,
        ...
        const destination = path.join(
          isMovieMode ? this.moviesDir : this.showsDir,
          isMovieMode ? `${title} (${year})` : title,
          isMovieMode
            ? `${title} (${year})${ext}`
